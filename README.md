...
- **Euphotic layer depth** (*z<sub>eu</sub>*) monthly climatology. I constructed this climatology using the light attenuation coefficient ([k<sub>d</sub>](https://data.marine.copernicus.eu/product/OCEANCOLOUR_GLO_BGC_L4_MY_009_104/description)) from Copernicus Marine Service (CMEMS) L4 biogeochemical product and the 1% PAR<sub>0</sub> criterion. The file, `zeu_calculated_onepercentpar0.mat`, is necessary to run the functions for fitting Martin's *b* and the remineralisation length scale coefficient (*z**).
